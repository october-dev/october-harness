...
import { getAgentDir, getDocsPath, getExamplesPath, getReadmePath } from "@october-dev/october";
	...
	it("removes runner metadata and restores the process environment", () => {
		...
		try {
			...
			try {
				...
				expect(process.env.OCTOBER_CODING_AGENT_DIR).toBe("/tmp/eval-agent");
				expect(getAgentDir()).toBe("/tmp/eval-agent");
...
describe("documentation variant", () => {
	...
	it("strips only the documentation routing section from the default Pi prompt", () => {
		...
		expect(stripped).not.toContain("October Harness documentation");
