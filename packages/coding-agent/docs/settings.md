...
| `enableInstallTelemetry` | boolean | `false` | Send an anonymous install/update version ping after first install or changelog-detected updates. Off by default. This does not control update checks |
...
`enableInstallTelemetry` only controls the anonymous install/update ping to `https://www.october.dev/api/cli/report-install` (off by default). Opting out of telemetry does not disable update checks; October can still fetch `https://www.october.dev/api/cli/latest-version` to look for the latest version.

Set `PI_SKIP_VERSION_CHECK=1` to disable the October version update check. Use `--offline` or `PI_OFFLINE=1` to disable all startup network operations described here, including update checks, package update checks, and install/update telemetry.
