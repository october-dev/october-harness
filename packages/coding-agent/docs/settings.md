...
For VS Code, set `externalEditor` to `"code --wait"` so October resumes after the editor exits.
...
| Setting | Type | Default | Description |
|---|---|---|---|
| `sessionDir` | string | Agent session directory | Session storage directory. Relative paths resolve from the working directory. `OCTOBER_CODING_AGENT_SESSION_DIR` and `--session-dir` override this setting. |
...
Resource paths in user settings resolve from the agent directory. Paths in project settings resolve from the project `.october` directory. Absolute paths and `~` are supported.
...
| `enableInstallTelemetry` | boolean | `false` | Enable anonymous install/update reporting and selected provider attribution headers. Off by default in October. Does not control update checks. |
| `enableAnalytics` | boolean | `false` | Opt in to analytics data sharing. Currently used only by the experimental first-run setup. |
| `trackingId` | string | None | Analytics tracking identifier, generated when `enableAnalytics` is turned on. |
| `warnings.anthropicExtraUsage` | boolean | `true` | Warn when Anthropic subscription authentication may use paid extra usage. |

October's anonymous install/update reporting uses `https://www.october.dev/api/cli/report-install`. Opting out does not disable the latest-version check against npm's published `@october-dev/october` metadata. Set `PI_SKIP_VERSION_CHECK=1` to disable the background version check, or `PI_OFFLINE=1` / `--offline` to disable automatic network activity, including update checks, package updates, and install/update telemetry.
