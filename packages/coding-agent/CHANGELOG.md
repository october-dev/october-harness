...
- Synced upstream Pi through `8d897edaa`, including Pi 0.87.1 model support, durable storage, Chord state tracking, documentation updates, and extension runtime fixes, while preserving October's package identity, authentication, Bus integration, permissions, and release behavior.
...
- Prevented model-edited or untrusted project settings from relaxing October tool permissions during a session.
- Preserved explicit inference tokens when Desktop credentials are cleared.
- Rejected mismatched JSON-RPC IDs and failed HTTP responses; bounded Bus response memory and handled SSE results without waiting for connection closure.
...
- Merged upstream pi 0.84.3 (117 commits) and reset the October counter to `0.84.3-october.1`.
- Added RPC `clear_queue` to retrieve and remove queued steering and follow-up messages ([#8432](https://github.com/earendil-works/pi/issues/8432)).
...
- Changed the Node.js CLI and RPC entrypoints to load a bundled runtime, reducing startup filesystem reads while keeping the public library and legacy module paths on the modular runtime for normal dependency identity. The `october` binary still points at `dist/cli.js` (Desktop Part D).
...
- Rebranded the coding-agent binary to `october` (package `@october-dev/october`, config dir `.october`).
...
- Hardened Desktop October credentials: the injected session stays in memory (not auth.json). Because that skips pi's stored-oauth refresh loop, the extension refreshes the JWT itself on `session_start` / `before_agent_start` (and on a timer) via `GET /auth/october-token`, falling back to Supabase only while that route has never succeeded. After the first bus refresh, later bus failures keep the last-good token instead of POSTing the spawn-time refresh token (Supabase reuse-detection would sign out everywhere). `$OCTOBER_INFERENCE_TOKEN` is re-read from the environment on each request. `OCTOBER_SUPABASE_USER_ID` replaces a mismatched stored identity. Failed store writes are surfaced instead of swallowed.
- Logged every `/v1/models` seed-fallback reason (`no token`, `HTTP 401`, `bad shape`, …) and every inference HTTP 401 to the October debug log so a two-model catalogue is diagnosable. `october --version` / `-v` now print the package version and exit 0 from the zero-import preflight, so Desktop's version gate works on Node < 22.19; other argv still fail closed on old Node. Bare `october --help` is no longer captured by the login subcommand. Device-code login now polls locally instead of importing an unpublished `@earendil-works/pi-ai/oauth` runtime export (that import crashed `--help` and `october login` against published pi-ai 0.84.2).
