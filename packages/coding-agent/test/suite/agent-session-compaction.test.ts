...
describe("AgentSession compaction characterization", () => {
	...
	it.each([false, true])(
		...
		async (modelOverride) => {
			...
			const harness = await createHarness({
				models: [{ id: "faux-1", contextWindow: 3600, maxTokens: 100 }],
				settings: {
					compaction: modelOverride
						? {
								...
								modelOverrides: { "faux/faux-1": { reserveTokens: 1400, keepRecentTokens: 1750 } },
							}
						: { enabled: true, reserveTokens: 1400, keepRecentTokens: 1750 },
				},
				tools: [largeTool],
				extensionFactories: [
					(pi) => {
			...
			expect(observedSettings[0]).toEqual({ enabled: true, reserveTokens: 1400, keepRecentTokens: 1750 });
