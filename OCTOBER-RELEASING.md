...
The build jobs upload checksummed, attested payloads before the separate `publish` job runs. The publisher downloads those exact payloads; it never rebuilds or repacks them. npm integrity verification retries boundedly to allow newly published registry metadata to propagate, but a different integrity or package identity fails immediately.
