...
The October candidate version is `0.87.1-october.2`. Its upstream dependency versions stay on Pi 0.87.1. Keep the October dependency in `packages/evals/package.json` aligned when changing the October version. Refresh the root lockfile without scripts and regenerate the coding-agent shrinkwrap/install lock using the repository helpers.
