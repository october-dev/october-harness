...
#     october-darwin-arm64.tar.gz
#     october-darwin-x64.tar.gz
#     october-linux-x64.tar.gz
#     october-linux-arm64.tar.gz
#     october-windows-x64.zip
#     october-windows-arm64.zip
...
for platform in "${PLATFORMS[@]}"; do
    ...
    if [[ "$platform" == windows-* ]]; then
        bun build --compile --no-compile-autoload-bunfig --target="$bun_target" ./dist/bun/cli.js ./src/utils/image-resize-worker.ts --outfile "$OUTPUT_DIR/$platform/october.exe"
    else
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        # Windows (zip)
        echo "Creating october-$platform.zip..."
        (cd "$platform" && zip -r ../october-$platform.zip .)
    else
        # Unix platforms (tar.gz) - use wrapper directory for mise compatibility
        echo "Creating october-$platform.tar.gz..."
        mv "$platform" october && tar -czf october-$platform.tar.gz october && mv october "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    rm -rf "$platform"
    if [[ "$platform" == windows-* ]]; then
        mkdir -p "$platform" && (cd "$platform" && unzip -q ../october-$platform.zip)
    else
        tar -xzf october-$platform.tar.gz && mv october "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        echo "  $OUTPUT_DIR/$platform/october.exe"
    else
        echo "  $OUTPUT_DIR/$platform/october"
    fi
done
